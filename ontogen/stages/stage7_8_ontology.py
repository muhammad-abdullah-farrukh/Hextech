...
def _extract_turtle(raw: str) -> str:
    """Pull the Turtle block out of an LLM response.

    A reasoning model at a generous max_tokens sometimes prefaces the
    answer with an explanatory sentence before a ```turtle fenced block
    (e.g. "To build the ontology, I'll follow ... \n\n```turtle\n@prefix...").
    Blindly stripping fence markers leaves that prose in place and breaks
    rdflib parsing, so prefer the fenced block's contents when present and
    only fall back to the raw text otherwise.
    """
    m = re.search(r"```(?:turtle)?\s*\n(.*?)```", raw, re.DOTALL)
...
def _new_prop_turtle(extracted: dict) -> str:
    ...
    # max_tokens=3000 (was 300 — reasoning model burned the entire budget on
    # reasoning before ever emitting Turtle, 0 chars back every time; 3000
    # was chosen from measured completion_tokens of 843-1368 across real
    # properties from this prompt shape, ~2.2x margin, matching the
    # precedent bump already made in stage9_10_kg.py for the same failure).
    raw = call_llm(prompt, max_tokens=3000)
    print(f"[Stage 7/8]   ✓ got {len(raw)} chars back", flush=True)
    raw = _extract_turtle(raw)
